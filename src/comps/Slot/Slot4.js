...
function Slot() {
  ...
  function spin() {
    ...
    setRow2(false);
    setRow3(false);
    let nu1 = getRandomInt() % 10;
    let nu2 = getRandomInt() % 10;
    let nu3 = getRandomInt() % 10;
    let nu4 = getRandomInt() % 10;
    let nu5 = getRandomInt() % 10;
    let nu6 = getRandomInt() % 10;
    let nu7 = getRandomInt() % 10;
    let nu8 = getRandomInt() % 10;
    let nu9 = getRandomInt() % 10;
    let nu10 = getRandomInt() % 10;
    let nu11 = getRandomInt() % 10;
    let nu12 = getRandomInt() % 10;
    let nu13 = getRandomInt() % 10;
    let nu14 = getRandomInt() % 10;
    let nu15 = getRandomInt() % 10;
    switch (nu1) {
      case 0:
        setCol11("\ud83c\udf47");
        break;
  ...
  useEffect(() => {
    if (row1 === true && row2 === true && row3 === true) {
      ...
      updateScoreContext(user.id, user.email, token, 10000 * point);
      updateStatContext(user.id, user.email, token, jackpot);
    } else if (row1 === true) {
      alertify.success("ROW1! +1000", 1);
      updateScoreContext(user.id, user.email, token, 100 * point);
      updateStatContext(user.id, user.email, token, game);
    } else if (row2 === true) {
      alertify.success("ROW2! +1000", 1);
      updateScoreContext(user.id, user.email, token, 100 * point);
      updateStatContext(user.id, user.email, token, game);
    } else if (row3 === true) {
      alertify.success("ROW3! +1000", 1);
      updateScoreContext(user.id, user.email, token, 100 * point);
      updateStatContext(user.id, user.email, token, game);
    }
  }, [row1, row2, row3]);
